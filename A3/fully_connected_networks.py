...
class Linear(object):
    ...
    def forward(x, w, b):
        ...
        N = x.shape[0]
        M = b.shape[0]
        out = torch.zeros(N, M) # create matrix to fill

        x_flatten = x.reshape(N, -1) # flatten dimensions in D
        out = x_flatten.mm(w) + b.reshape(1, M) # forward pass
        ######################################################################
        #                        END OF YOUR CODE                            #
        ######################################################################
        cache = (x, w, b)
    ...
    def backward(dout, cache):
        ...
        N = x_dims[0]
        M = b.shape[0]
        x_flatten = x.reshape(N, -1) # flatten x for gradient computation
        ...
        # reshape dx for output
        dx = dx.reshape(x_dims)
...
class TwoLayerNet(object):
    ...
    def __init__(self, input_dim=3*32*32, hidden_dim=100, num_classes=10,
        ...
        ###################################################################
        self.params['W1'] = torch.empty(input_dim, hidden_dim, dtype=dtype, device=device).normal_(mean=0, std=weight_scale)
        self.params['b1'] = torch.zeros(hidden_dim, dtype=dtype, device=device)
        self.params['W2'] = torch.empty(hidden_dim, num_classes, dtype=dtype, device=device).normal_(mean=0, std=weight_scale)
    ...
    def loss(self, X, y=None):
        ...
        W2 = self.params['W2']
        # hidden layer w/ ReLU
        hidden, hidden_cache = Linear_ReLU.forward(X, W1, self.params['b1'])
        # scores 
        scores, scores_cache = Linear.forward(hidden, W2, self.params['b2'])
        # data loss
        loss, d_scores = softmax_loss(scores, y) 

        # regularization loss
        reg_loss = self.reg*torch.sum(W1**2) + self.reg*torch.sum(W2**2)
        # total loss
        loss += reg_loss
        ##############################################################
        #                     END OF YOUR CODE                       #
        ...
        ###################################################################
        # data gradient
        d_hidden, d_W2, d_b2 = Linear.backward(d_scores, scores_cache)
        d_X, d_W1, d_b1 = Linear_ReLU.backward(d_hidden, hidden_cache)
        # regularization gradient
        d_W2 += 2 * self.reg * W2
        d_W1 += 2 * self.reg * W1
        # place in grads dictionary
        grads['W1'] = d_W1 
        grads['b1'] = d_b1 
        grads['W2'] = d_W2 
        grads['b2'] = d_b2
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        count = 0
        # first layer
        self.params['W0'] = torch.empty(input_dim, hidden_dims[0], dtype=dtype, device=device).normal_(mean=0, std=weight_scale)
        self.params['b0'] = torch.zeros(hidden_dims[0], dtype=dtype, device=device)
        # rest of hidden layers
        for i in range(1, self.num_layers-1): 
          self.params['W{}'.format(i)] = torch.empty(hidden_dims[i-1], hidden_dims[i], dtype=dtype, device=device).normal_(mean=0, std=weight_scale)
          self.params['b{}'.format(i)] = torch.zeros(hidden_dims[i], dtype=dtype, device=device)
          count = i
        # final layer
        self.params['W{}'.format(count+1)] = torch.empty(hidden_dims[count], num_classes, dtype=dtype, device=device).normal_(mean=0, std=weight_scale)
        self.params['b{}'.format(count+1)] = torch.zeros(num_classes, dtype=dtype, device=device)
        #######################################################################
        #                         END OF YOUR CODE                            #
        #######################################################################
    ...
    def loss(self, X, y=None):
        ...
        ##################################################################
        caches = [] # store weights
        dropout_caches = [] # store dropout masks
        prev_mat = X
        reg_loss = 0

        # compute forward pass except last layer
        count = 0
        for i in range(self.num_layers-1): 
          cur_W = self.params['W{}'.format(i)]
          cur_b = self.params['b{}'.format(i)]
          cur_mat, cur_cache = Linear_ReLU.forward(prev_mat, cur_W, cur_b)
          # dropout
          if self.use_dropout: 
            cur_mat, cur_drop_cache = Dropout.forward(cur_mat, self.dropout_param)
            dropout_caches.append(cur_drop_cache)

          caches.append(cur_cache)
          # regularization loss
          reg_loss += self.reg * torch.sum(cur_W**2)
          # next matrix
          prev_mat = cur_mat
          count = i

        # compute last layer
        last_W = self.params['W{}'.format(count+1)]
        last_b = self.params['b{}'.format(count+1)]
        scores, scores_cache = Linear.forward(prev_mat, last_W, last_b)
        ...
        loss, d_scores = softmax_loss(scores, y)
        # reg loss
        reg_loss += self.reg * torch.sum(last_W**2)
        # total loss        
        loss += reg_loss

        # backprop, start with last layer
        prev_dout, dW_last, db_last = Linear.backward(d_scores, scores_cache)
        dW_last += 2 * self.reg * self.params['W{}'.format(count+1)]
        dW_last += 2 * self.reg * self.params['b{}'.format(count+1)]
        # reg loss
        grads['W{}'.format(count+1)] = dW_last
        grads['b{}'.format(count+1)] = db_last
        # backprop for all previous layers
        for i in range(count, -1, -1):
          # dropout
          if self.use_dropout: 
            prev_dout = Dropout.backward(prev_dout, dropout_caches[i])
          prev_dout, cur_dW, cur_db = Linear_ReLU.backward(prev_dout, caches[i])
          # reg loss
          cur_dW += 2 * self.reg * self.params['W{}'.format(i)]
          cur_db += 2 * self.reg * self.params['b{}'.format(i)]
          grads['W{}'.format(i)] = cur_dW
          grads['b{}'.format(i)] = cur_db
        ###########################################################
        #                   END OF YOUR CODE                      #
        ###########################################################
