...
def svm_loss_vectorized(
    ...
    #############################################################################
    num_train = X.shape[0]
    scores = W.t().mm(X.t())  # get raw scores (NxC) 
    correct_class_score = scores[y.t(), range(num_train)]  # correct scores for each class (Nx1)
    # use broadcasting: (NxC - Nx1)
    loss_mat = scores - correct_class_score + 1  # margin scores ; delta equals 1
    loss_mat[y.t(), range(num_train)] = 0  # set indices (j == y[i]) = 0
    loss_mat[loss_mat < 0] = 0  # set negatives to 0 (max function)

    loss = torch.sum(loss_mat) / num_train + reg * torch.sum (W*W)
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
...
def sample_batch(
    ...
    sample = torch.randint(high=num_train, size=(batch_size,))
    X_batch = X[sample]
    y_batch = y[sample]
    #########################################################################
    #                       END OF YOUR CODE                                #
...
def predict_linear_classifier(W: torch.Tensor, X: torch.Tensor):
    ...
    # Replace "pass" statement with your code
    scores = W.t().mm(X.t())
    vals, idxs = torch.max(scores, dim=0)
    y_pred = idxs
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
...
def svm_get_search_params():
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    learning_rates = [5e-1, 1e-1, 5e-2, 1e-2, 5e-4]
    regularization_strengths = [1e-10, 1e-8, 1e-6, 1e-4, 1e-2]
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
...
def softmax_loss_naive(
    ...
    for i in range(N): 
      ...
      e_yi = 0  # correct label value
      e_fi_sum = 0  # sum of all labels
      # get correct values and sum of incorrect values
      for j in range(C):
        if j == y[i]: 
          e_yi = e_f[i][j]
        e_fi_sum += e_f[i][j]
      # add loss for each example
      loss += -torch.log(e_yi / e_fi_sum)
      # calculate gradient
      for k in range(C): 
        if k == y[i]: 
          dW[k] += X[i] * (e_yi / e_fi_sum - 1)
        else: 
          dW[k] += X[i] * (e_f[i][k] / e_fi_sum)
...
def softmax_loss_vectorized(
    ...
    # Initialize the loss and gradient to zero.
    loss = 0.0
    dW = torch.zeros_like(W).t() # I like the transpose shape more for analytic reasons
    ...
    # normalize f
    f -= torch.max(f)
    # raise e to each matrix value
    e_f = torch.exp(f)

    # LOSS
    # sum each e_f vals for all classes in each example
    e_fi_sum = torch.sum(e_f, dim=1)
    # get e_f vals for the correct class
    e_yi = e_f[range(N), y]
    # compute loss for each example and average
    loss = torch.sum( torch.log(e_fi_sum / e_yi ) ) / N + reg*torch.sum(W*W)

    # GRADIENT
    
    # gradient for W_j
    # clone scores matrix
    e_f_cp = e_f.clone() # shape (NxC)
    # set correct class scores to 0
    e_f_cp[range(N), y] = 0 # shape (NxC)
    # compute last thing to multiply by examples
    k = e_f_cp / e_fi_sum.view(N, 1)
    # add to gradient
    dW += k.t().mm(X) / N

    # gradient for W_yi
    # use mask to get rid of all incorrect class scores
    e_f_mask = torch.zeros_like(e_f)
    # define mask 
    e_f_mask[range(N), y] = 1
    # get rid of incorrect class scores
    e_f_yi = e_f * e_f_mask
    # compute last thing to multiply be examples
    k = (e_f_yi / e_fi_sum.view(-1, 1)) - e_f_mask
    # add to gradient
    dW += k.t().mm(X) / N
